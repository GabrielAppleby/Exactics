...
public sealed class CreateGameMapSystem : IInitializeSystem {
	...
	public void Initialize() {
		...
		_gameContext.SetGameMap(numTilesWide, numTilesHigh);
		...
		foreach (Transform child in mapTransform) {
			if (child.name != "Terrain") {
				...
				foreach (Transform nestedChild in child) {
					tileEntity = _gameContext.CreateEntity();
					tileEntity.AddPosition(nestedChild.position.x, nestedChild.position.y);
					tileEntity.isGameMapElement = true;
					tileEntity.isInteractive = true;
					tileEntity.isImpassable = true;
	...
	private sealed class CoordinateCompare : Comparer<GameEntity>  {
		public override int Compare(GameEntity one, GameEntity two)
		{
